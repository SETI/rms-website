...
PROD_DIR = '/var/www/documents/'  # full path to web root directory including trailing slash

REPO = 'https://github.com/SETI/pds-website.git'
branch = 'master'

ADMIN_HOST = 'staging.pds.seti.org'
...
env.hosts = [ADMIN_HOST, SERVER1_HOST, SERVER2_HOST]
...
def admin(suffix=""):
    ...
    admin_website = f'https://{ADMIN_HOST}'

    # get the latest from github
    with lcd(ADMIN_REPO):
        local(f'git checkout {branch}')
        if git_revision:
            local(f'git checkout {git_revision}')
        local('git pull')

    # build the site and then move into web root
    with lcd(f'{ADMIN_REPO}website{suffix}/'):
