...
1. Using your own clone of the rms-website repo on your personal computer, edit the necessary files, convert them with jekyll (if necessary), and use a local webserver to test the results.
...
5. Merge your branch onto the main branch.
...
This is the basic flow for making changes to the website files that are stored in git in the rms-website repo. While the steps listed below appear to be a strict chronology, more complicated changes may require multiple iterations through subsets of steps or working on some steps out of order. It is assumed that you already have a basic knowledge of jekyll, git, and GitHub.
...
1. First time:
    ...
    2. Create a clone of the rms-website repo on your local computer:

        ```
        git clone https://github.com/SETI/rms-website.git
        ```
...
8. Create a pull request on GitHub
    1. Navigate your browser to: [https://github.com/SETI/rms-website/pulls](https://github.com/SETI/rms-website/pulls)
...
10. Once the pull request is approved, merge your changes onto the main branch by going to the pull request page on GitHub and selecting “Merge pull request”.

### Deploy the Changes to the Production Servers

This procedure will deploy the current “main” branch on the rms-website repo to staging, then copy them to `/volumes/webserver_assets`, then deploy them to `server1` and `server2`.
...
2. Deploy changes to the local apache directory. This script will automatically pull the changes from GitHub and checkout the **main** branch.
...
NOTE: This will deploy the _current contents_ of the `rms-website/_site` directory without updating it from GitHub or checking out a branch or running jekyll gain. Make sure the `website` directory is in the state you want it to be in, ideally by performing the above steps first.
The steps performed by this step are:
    1. Show the differences between the `rms-website/_site` and `/volumes/webserver_assets/documents` directories
    2. Copy the contents of `rms-website/_site` to `/volumes/webserver_assets/documents`
...
Certain parts of the website (specifically the Reviews directory, containing websites intended for doing the work of peer review that are then kept for posterity) are not currently part of the Jekyll workflow.  We edit these HTML files directly, rather than using Markdown.  **However,** these website source files are still in the `rms-website` repo!
...
To sync the local `/var/www/documents` with `/volumes/webserver_assets`:
  ...
  ```
  cd ~webmaster/rms-website/deploy
